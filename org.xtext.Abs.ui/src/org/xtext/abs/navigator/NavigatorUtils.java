...
public abstract class NavigatorUtils {
	...
	private static void openAndHighlightEditor(InternalASTNode<?> node) throws PartInitException {
		ITextEditor editorPart = openABSEditorForFile(node.getFileName(), node.getProject());
		//@@@@@@@@@@@@@ToDoInEditor//editorPart.highlightInEditor(node, true);
	}	
	
	public static void updateDependencies(TreeSelection ts) {
		IProject project = getProject(ts);
		if (project != null) {
			SampleABSNature nature = getAbsNature(project);
	 ...
	 */
	public static boolean hasSubModules(ModuleDecl m, SampleABSNature nature) {
